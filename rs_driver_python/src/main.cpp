...
py::array_t<float> points_to_numpy_xyzi(const PointCloud_XYZI &point_cloud) {
    ...
    {
        unchecked_ref(row, 0) = point_cloud.points[row].x;        // avanti
        unchecked_ref(row, 1) = point_cloud.points[row].y;        // sinistra
        unchecked_ref(row, 2) = point_cloud.points[row].z;        // su
...
PYBIND11_MODULE(rs_driver, m) {
    ...
    py::class_<PointCloud_XYZI, std::shared_ptr<PointCloud_XYZI>>(m, "PointCloud")
        ...
        .def("__str__", [](const PointCloud_XYZI &a) {
             return "Point cloud " + std::to_string(a.seq) + " of size " + std::to_string(a.points.size());
        })
    ...
    // TODO: Explicitly wrap driver for every possible generic type
